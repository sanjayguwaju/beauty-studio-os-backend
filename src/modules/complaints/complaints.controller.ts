...
export async function createComplaint(req: AuthRequest, res: Response) {
  ...
  try {
    ...
    ["platform_admin", "municipality_admin", "ward_officer", "section_head"].forEach((role) => {
...
export async function getComplaint(req: AuthRequest, res: Response) {
  const c = await Complaint.findOne({ _id: req.params.id, municipalityId: req.user!.municipalityId });
