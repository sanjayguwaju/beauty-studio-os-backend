...
export async function listSchools(req: AuthRequest, res: Response) {
  ...
  const [data, total] = await Promise.all([
    School.find(filter).populate("wardId", "wardNumber").skip((page - 1) * pageSize).limit(pageSize).sort({ name: 1 }),
...
export async function createSchool(req: AuthRequest, res: Response) {
  const school = await School.create({ ...req.body, municipalityId: req.user!.municipalityId });
  await AuditLog.create({ municipalityId: req.user!.municipalityId, actorId: req.user!.id, actorEmail: req.user!.email, module: "education", action: "CREATE", entityType: "School", entityId: school._id, entityLabel: school.name, after: school.toObject() });
  return sendSuccess(res, school, "School created", 201);
}

export async function getSchool(req: AuthRequest, res: Response) {
  const school = await School.findOne({ _id: req.params.id, municipalityId: req.user!.municipalityId, isDeleted: false }).populate("wardId", "wardNumber nameNp");
...
export async function updateSchool(req: AuthRequest, res: Response) {
  const before = await School.findOne({ _id: req.params.id, municipalityId: req.user!.municipalityId, isDeleted: false });
  ...
  await AuditLog.create({ municipalityId: req.user!.municipalityId, actorId: req.user!.id, actorEmail: req.user!.email, module: "education", action: "UPDATE", entityType: "School", entityId: school!._id, entityLabel: school!.name, before: before.toObject(), after: school!.toObject() });
  return sendSuccess(res, school, "School updated");
}

export async function deleteSchool(req: AuthRequest, res: Response) {
  const school = await School.findOneAndUpdate({ _id: req.params.id, municipalityId: req.user!.municipalityId }, { isDeleted: true }, { new: true });
  if (!school) return sendError(res, 404, "School not found");
  await AuditLog.create({ municipalityId: req.user!.municipalityId, actorId: req.user!.id, actorEmail: req.user!.email, module: "education", action: "DELETE", entityType: "School", entityId: school._id, entityLabel: school.name });
...
export async function listSchoolPerformance(req: AuthRequest, res: Response) {
  const school = await School.findOne({ _id: req.params.id, municipalityId: req.user!.municipalityId, isDeleted: false });
  if (!school) return sendError(res, 404, "School not found");
