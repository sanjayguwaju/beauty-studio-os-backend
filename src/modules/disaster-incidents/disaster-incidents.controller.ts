...
export async function listIncidents(req: AuthRequest, res: Response) {
  ...
  const [data, total] = await Promise.all([
    DisasterIncident.find(filter).populate("wardId", "wardNumber").populate("reportedBy", "name").skip((page - 1) * pageSize).limit(pageSize).sort({ createdAt: -1 }),
...
export async function createIncident(req: AuthRequest, res: Response) {
  const body = { ...req.body, municipalityId: req.user!.municipalityId, reportedBy: req.user!.id };
  ...
  await AuditLog.create({ municipalityId: req.user!.municipalityId, actorId: req.user!.id, actorEmail: req.user!.email, module: "disaster_management", action: "CREATE", entityType: "DisasterIncident", entityId: incident._id, entityLabel: `${incident.type} — ${incident.severity}`, after: incident.toObject() });
  return sendSuccess(res, incident, "Incident reported", 201);
}

export async function getIncident(req: AuthRequest, res: Response) {
  const incident = await DisasterIncident.findOne({ _id: req.params.id, municipalityId: req.user!.municipalityId, isDeleted: false }).populate("wardId", "wardNumber").populate("reportedBy", "name email");
...
export async function updateIncident(req: AuthRequest, res: Response) {
  const before = await DisasterIncident.findOne({ _id: req.params.id, municipalityId: req.user!.municipalityId, isDeleted: false });
  ...
  await AuditLog.create({ municipalityId: req.user!.municipalityId, actorId: req.user!.id, actorEmail: req.user!.email, module: "disaster_management", action: "UPDATE", entityType: "DisasterIncident", entityId: incident!._id, entityLabel: `${incident!.type}`, before: before.toObject(), after: incident!.toObject() });
  return sendSuccess(res, incident, "Incident updated");
}

export async function deleteIncident(req: AuthRequest, res: Response) {
  const incident = await DisasterIncident.findOneAndUpdate({ _id: req.params.id, municipalityId: req.user!.municipalityId }, { isDeleted: true }, { new: true });
  if (!incident) return sendError(res, 404, "Incident not found");
  await AuditLog.create({ municipalityId: req.user!.municipalityId, actorId: req.user!.id, actorEmail: req.user!.email, module: "disaster_management", action: "DELETE", entityType: "DisasterIncident", entityId: incident._id, entityLabel: incident.type });
...
export async function listDamageAssessments(req: AuthRequest, res: Response) {
  const incident = await DisasterIncident.findOne({ _id: req.params.id, municipalityId: req.user!.municipalityId, isDeleted: false });
  if (!incident) return sendError(res, 404, "Incident not found");
...
export async function createDamageAssessment(req: AuthRequest, res: Response) {
  ...
  await AuditLog.create({ municipalityId: req.user!.municipalityId, actorId: req.user!.id, actorEmail: req.user!.email, module: "disaster_management", action: "CREATE", entityType: "DamageAssessment", entityId: assessment._id, entityLabel: assessment.description });
