...
export async function getCachedPersonSession(personId: string) {
  const cacheKey = `session:person:${personId}`;
  const cached = await redis.get(cacheKey);
  ...
  const person = await Person.findById(personId).select("-password -refreshToken").lean();
  ...
  const sessionData = { person, roleAssignments };
  await redis.set(cacheKey, JSON.stringify(sessionData), 'EX', 900);
  return sessionData;
}
