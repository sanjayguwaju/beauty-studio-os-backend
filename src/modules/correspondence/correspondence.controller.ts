...
async function nextSeq(municipalityId: string, fiscalYear: string): Promise<number> {
  const key = `${municipalityId}-${fiscalYear}`;
  if (!seqCache[key]) {
    const last = await Correspondence.findOne({ municipalityId, fiscalYear }).sort("-sequenceNumber");
...
export async function createCorrespondence(req: AuthRequest, res: Response) {
  ...
  const seq = await nextSeq(req.user!.municipalityId, fiscalYear);
...
export async function getCorrespondence(req: AuthRequest, res: Response) {
  const c = await Correspondence.findOne({ _id: req.params.id, municipalityId: req.user!.municipalityId });
