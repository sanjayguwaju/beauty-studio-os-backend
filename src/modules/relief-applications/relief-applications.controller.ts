...
export async function createReliefApplication(req: AuthRequest, res: Response) {
  // Verify incident belongs to this municipality
  const incident = await DisasterIncident.findOne({ _id: req.body.incidentId, municipalityId: req.user!.municipalityId, isDeleted: false });
  ...
  const approval = await createApproval(req.user!.municipalityId, "disaster_management", "ReliefApplication", application._id as Types.ObjectId, actorLevel);
  await ReliefApplication.findByIdAndUpdate(application._id, { approvalId: approval._id });

  await AuditLog.create({ municipalityId: req.user!.municipalityId, actorId: req.user!.id, actorEmail: req.user!.email, module: "relief_applications", action: "CREATE", entityType: "ReliefApplication", entityId: application._id, entityLabel: application.applicantName, after: application.toObject() });
...
export async function updateReliefApplication(req: AuthRequest, res: Response) {
  ...
  await AuditLog.create({ municipalityId: req.user!.municipalityId, actorId: req.user!.id, actorEmail: req.user!.email, module: "relief_applications", action: "UPDATE", entityType: "ReliefApplication", entityId: application!._id, entityLabel: application!.applicantName, before: before.toObject(), after: application!.toObject() });
