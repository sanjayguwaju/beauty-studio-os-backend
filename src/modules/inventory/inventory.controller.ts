...
export async function updateItem(req: AuthRequest, res: Response) {
  ...
  const item = await InventoryItem.findOneAndUpdate(
    { _id: req.params.id, municipalityId: req.user!.municipalityId, module, isDeleted: false },
    { $set: req.body },
    { new: true },
  );
  if (!item) return sendError(res, 404, "Item not found");
...
export async function deleteItem(req: AuthRequest, res: Response) {
  const module = getModule(req);
  const item = await InventoryItem.findOneAndUpdate(
    { _id: req.params.id, municipalityId: req.user!.municipalityId, module },
    { isDeleted: true },
    { new: true },
  );
  if (!item) return sendError(res, 404, "Item not found");
...
export async function createTransaction(req: AuthRequest, res: Response) {
  // Verify item belongs to this municipality and module
  const module = getModule(req);
  const item = await InventoryItem.findOne({
    _id: req.body.itemId,
    municipalityId: req.user!.municipalityId,
...
export async function listDistributions(req: AuthRequest, res: Response) {
  ...
  const [data, total] = await Promise.all([
    Distribution.find({ municipalityId: req.user!.municipalityId })
      .populate({ path: "transactionId", populate: { path: "itemId", select: "name unit" } })
      .skip((page - 1) * pageSize)
      .limit(pageSize)
      .sort({ createdAt: -1 }),
    Distribution.countDocuments({ municipalityId: req.user!.municipalityId }),
  ]);
  return sendPaginated(res, data, total, page, pageSize);
}
