...
export async function createAdminDocument(req: AuthRequest, res: Response) {
  ...
  const approval = await createApproval(req.user!.municipalityId, "administrative", "AdminDocument", doc._id as Types.ObjectId, actorLevel);
  await AdminDocument.findByIdAndUpdate(doc._id, { approvalId: approval._id });

  await AuditLog.create({ municipalityId: req.user!.municipalityId, actorId: req.user!.id, actorEmail: req.user!.email, module: "administrative", action: "CREATE", entityType: "AdminDocument", entityId: doc._id, entityLabel: doc.title, after: doc.toObject() });
...
export async function getAdminDocument(req: AuthRequest, res: Response) {
  const doc = await AdminDocument.findOne({ _id: req.params.id, municipalityId: req.user!.municipalityId, isDeleted: false }).populate("createdBy", "name").populate("approvalId");
...
export async function updateAdminDocument(req: AuthRequest, res: Response) {
  const before = await AdminDocument.findOne({ _id: req.params.id, municipalityId: req.user!.municipalityId, isDeleted: false });
  ...
  await AuditLog.create({ municipalityId: req.user!.municipalityId, actorId: req.user!.id, actorEmail: req.user!.email, module: "administrative", action: "UPDATE", entityType: "AdminDocument", entityId: doc!._id, entityLabel: doc!.title, before: before.toObject(), after: doc!.toObject() });
  return sendSuccess(res, doc, "Document updated");
}

export async function deleteAdminDocument(req: AuthRequest, res: Response) {
  const doc = await AdminDocument.findOneAndUpdate({ _id: req.params.id, municipalityId: req.user!.municipalityId }, { isDeleted: true }, { new: true });
  if (!doc) return sendError(res, 404, "Document not found");
  await AuditLog.create({ municipalityId: req.user!.municipalityId, actorId: req.user!.id, actorEmail: req.user!.email, module: "administrative", action: "DELETE", entityType: "AdminDocument", entityId: doc._id, entityLabel: doc.title });
  return sendSuccess(res, null, "Document deleted");
}

export async function signDocument(req: AuthRequest, res: Response) {
  const doc = await AdminDocument.findOne({ _id: req.params.id, municipalityId: req.user!.municipalityId, isDeleted: false });
  if (!doc) return sendError(res, 404, "Document not found");
  ...
  await AuditLog.create({ municipalityId: req.user!.municipalityId, actorId: req.user!.id, actorEmail: req.user!.email, module: "admin_signatures", action: "SIGN", entityType: "AdminSignature", entityId: signature._id, entityLabel: doc.title });
